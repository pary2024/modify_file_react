    ...
    <div className={`container mx-auto p-4 ${isDark ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-900'}`}>
      ...
      <div className="flex flex-col md:flex-row justify-between mb-6 gap-4">
        ...
        <div className="bg-white dark:bg-gray-800 p-3 rounded shadow">
          <h3 className="font-medium text-gray-700 dark:text-gray-300">Total Provinces</h3>
          <p className="text-xl font-bold">{provinces.length}</p>
        </div>
      </div>

      {/* Province Form */}
      <form onSubmit={handleSave}  className="bg-white dark:bg-gray-800 p-4 rounded shadow mb-6">
              ...
              {provinces.length > 0 ? (
                provinces.map((province) => (
                  <tr key={province.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                    <td className="px-6 py-4 whitespace-nowrap font-medium">{province.name}</td>
                ...
                <tr>
                  <td colSpan="2" className="px-6 py-4 text-center text-gray-500 dark:text-gray-400">
                    {provinces.length === 0 ? "No provinces added yet" : "No matching provinces found"}
                  </td>
                </tr>
              )}
            </tbody>
